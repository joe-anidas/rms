<?php
// Simple database setup script with fixed timestamps
...
try {
    ...
    // Create tables with fixed timestamp columns
    $tables = [
    ...
    $sample_registrations = [
        [1, 1, '2024-01-15', 250000.00, 'completed', 'Initial registration for Green Valley Gardens'],
        [2, 2, '2024-02-01', 180000.00, 'pending', 'Booking for Sunrise Plots'],
        [3, 3, '2024-01-20', 320000.00, 'completed', 'Full registration for Paradise Gardens']
    ];
    
    foreach ($sample_registrations as $registration) {
        $stmt = $mysqli->prepare("INSERT INTO registrations (customer_id, property_id, registration_date, registration_amount, status, notes) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iisdss", ...$registration);
        if ($stmt->execute()) {
            echo "<p style='color: green;'>✓ Added registration for customer ID: {$registration[0]}</p>";
        } else {
            echo "<p style='color: red;'>✗ Error adding registration: " . $stmt->error . "</p>";
        }
    }
    
    // Sample transactions
    $sample_transactions = [
        [1, 1, 1, 'registration', 250000.00, 'bank_transfer', '2024-01-15', 'TXN001', 'Registration payment'],
        [2, 2, 2, 'booking', 50000.00, 'cash', '2024-02-01', 'TXN002', 'Booking amount'],
        [3, 3, 3, 'full_payment', 3200000.00, 'bank_transfer', '2024-01-20', 'TXN003', 'Full payment for property']
    ];
    
    foreach ($sample_transactions as $transaction) {
        $stmt = $mysqli->prepare("INSERT INTO transactions (customer_id, property_id, registration_id, transaction_type, amount, payment_method, payment_date, reference_number, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiisdssss", ...$transaction);
        if ($stmt->execute()) {
            echo "<p style='color: green;'>✓ Added transaction: {$transaction[6]}</p>";
        } else {
    ...
    $tables_to_check = ['customers', 'staff', 'properties', 'registrations', 'transactions'];
